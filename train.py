...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if accelerator.is_main_process and (not is_debug) and use_wandb:
        run = wandb.init(project="MagicAnimate", name=folder_name, config=config)
    ...
    max_train_steps = math.ceil(len(train_dataset) * max_train_epoch)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            pixel_values = batch["image"].to(local_rank, dtype=weight_type)
            ...
            random_number = secrets.randbelow(1001)
...
# python train.py --config configs/training/aa_train_stage1.yaml