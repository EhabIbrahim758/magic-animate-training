...
from MagicAnimate.magic_animate.context import (
...
class AnimationPipeline(DiffusionPipeline):
    ...
    #     control = controlnet_condition.to(device=device, dtype=self.controlnet.dtype)
    #     if do_classifier_free_guidance:
    ...
    #         controlnet_uncond_images, controlnet_cond_images = control.chunk(2)
    #     else:
    #         controlnet_cond_images = control.float()
    ...
    #     if latents is None:
    #         rand_device = "cpu" if device.type == "mps" else device
    #         latents = torch.randn(shape, generator=generator, device=rand_device, dtype=text_embeddings.dtype).to(device)
    ...
    #         controlnet_latent_input = self.scheduler.scale_model_input(latents, t)
            
    #         down_block_res_samples, mid_block_res_sample = self.controlnet(
    #             controlnet_latent_input,
    #             t,
    #             encoder_hidden_states=controlnet_text_embeddings_c,
    #             controlnet_cond=controlnet_cond_images.permute(2, 0, 1),
    #             conditioning_scale=controlnet_conditioning_scale,
    #             return_dict=False,
    #         )
    ...
    #         latent_model_input = (latents).to(device).repeat(2 if do_classifier_free_guidance else 1, 1, 1, 1, 1)
    #         latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
    ...
    def train(
        ...
        # latents should be of shape [1, 4, 1, 64, 64]
        latents_dtype = latents.dtype
